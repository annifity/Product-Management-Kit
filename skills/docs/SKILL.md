...
Reuse the supplied artifact, exact path, project, baseline, and requested operation. Ask only when target identity, permission to write, baseline impact, or destructive end state is material and unresolved. Never select a project template, baseline, or replacement path from filename recency alone.
...
1. Resolve the artifact-generation contract, project profile, material decisions, target, and internal write authority before writing.
...
10. Append decision, evidence, change, or traceability records when relevant and report exact paths, versions, and verification evidence. Keep machine audit details in machine records; explain stale or conflicting state in plain language unless the user explicitly requests technical diagnostics.
...
Return the requested artifact operation result in the user's language with exact target identity, path, version, lifecycle, baseline/latest pointer state, index impact, a concise generation result, and verification evidence. Do not expose raw hashes or resolver terminology by default. For a blocked or preview-only operation, explain the concrete blocker or planned effect without implying that a write occurred.

## Handoff

Return the exact artifact paths, versions, index links, concise localized generation result, and unresolved conflicts to the invoking workflow. State the action, target, source, baseline impact, and any decision needed without exposing technical audit values. Do not select a new product phase or change the owning workflow's decisions.
