...
- Reuse supplied context, including accepted design IDs and links, and do not ask for information already present.
...
7. Apply source-backed minimality; include dependencies, open questions, and out-of-scope work only when relevant and sourced. Before presenting an open question or requesting confirmation, apply the User Confirmation Clarity Gate in `_refs/checklists/material-decision-preflight.md`: write one plain-language product decision, explain its user-visible impact, offer concrete options when known, and route technical implementation choices to the technical owner.
8. On every revision, run the pruning pass in `_refs/checklists/source-backed-minimality.md` before returning the story: state each fact once across scope note, business context, and requirement notes; keep each change-log entry to one line of delta rather than re-narrating prior drafts; and replace a resolved open question with a one-line status instead of summarizing its resolution history a second time.
9. Ask `docs` to save or export the stories.
10. For Jira mutations, follow the draft, preview, explicit-approval, execution, and result-reporting gate in `_refs/integrations/jira.md`; Jira-ready or export wording alone is not mutation approval.
...
- Before authoring or slicing, resolve `_refs/schemas/artifact-generation-contract.md` through `_refs/operating-model/artifact-profile-resolution.md`, then use `_refs/checklists/material-decision-preflight.md`, including its User Confirmation Clarity Gate for user-facing decisions; before handoff, use `_refs/checklists/source-backed-minimality.md`.
- When deriving or revising stories from a governed PRD, spec, design handoff, plan, or story, resolve `_refs/operating-model/authoritative-baseline-resolution.md`; never choose by filename or date. Use `_refs/schemas/design-contract.md` only when accepted design IDs or unresolved design gaps affect the ticket.
