...
7. `strategy`: make or revise product and portfolio choices when evidence changes direction, investment, or sequencing across bets.
8. `spec`: convert a validated and strategically supported direction into working requirements when delivery is justified.
9. `design`: when user-visible behavior is material, turn the accepted spec
   into a traceable UX/UI contract and reviewable handoff.
10. `plan` / `execution` / `ship`: move through delivery planning, implementation support, and release when scope is ready.

## Skill Layers

- Flow skills: `discovery`, `brief`, `prototype`, `experiment`, `validate`,
  `learn`, `strategy`, `spec`, `design`, `plan`, `execution`, `ship`.
...
- If evidence must change product direction or allocate investment across bets, use `strategy`.
- If the team is ready to build, use `spec`, then `design` when user-visible
  behavior is material, then `plan`.
